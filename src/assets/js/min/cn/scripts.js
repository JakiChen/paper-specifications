...
var filterWidth = function() {
    ...
    convert = {
        ...
        tomm: function() {
            this.changeMeasurement(1), this.changeUnit("毫米"), this.changeLabels("毫米")
        },
        tocm: function() {
            this.changeMeasurement(.1), this.changeUnit("厘米"), this.changeLabels("厘米")
        },
        tom: function() {
            this.changeMeasurement(.001), this.changeUnit("米"), this.changeLabels("米")
        },
        toinches: function() {
            this.changeMeasurement(.0393701, "inches"), this.changeUnit("英寸"), this.changeLabels("英寸")
        },
        topoints: function() {
            this.changeMeasurement(2.835),
				this.changeUnit("点/磅 (pt)"),
				this.changeLabels("点/磅 (pt)");
        },
        topica: function() {
            this.changeMeasurement(0.2362),
				this.changeUnit("派卡 (pc)"),
				this.changeLabels("派卡 (pc)");
        },
        topixels: function() {
            this.changeMeasurement(25.4, "pixels"),
				this.changeUnit("像素 @300ppi"),
				this.changeLabels("像素 @300ppi");
        }
    },
...
var compareSelects = function() {
    ...
    var paper1 = {},
        ...
        mapDataCats = function(e) {
            ...
            $(".js-compare-categories").length &&
				($(".js-compare-categories").select2({
					placeholder: "选择一个类别...",
					data: t,
					width: "100%",
					minimumResultsForSearch: -1,
				...
				$(".js-compare-papers").on("select2:select", function (e) {
					var t = $(this);
					selectedPaperPicker(t, e.params.data.text, i),
						changeHistory(t, e.params.data.text);
				}));
        },
    ...
    $(".compare-paper-pickers").length && $.ajax({
        url: "/src/assets/data/zh_CN/paperpages.json",
        success: function(e) {
